...
public class KubernetesOperatorController {
  ...
  private static final long MAX_OPERATOR_STARTUP_TIME = 10;
  ...
  private UpgradeUniverseHandler upgradeUniverseHandler;
  ...
  public void run(long startTime) {
    ...
    //  List<HasMetadata> result = kubernetesClient.load(new
    // FileInputStream("../resources/universeCrd.yaml").get());
    //  // Apply Kubernetes Resources
    //  client.resourceList(result).inNamespace(namespace).createOrReplace();

    long elapsed = (System.currentTimeMillis() - startTime) / 1000;
    String elapsedStr = String.valueOf(elapsed);
    if (elapsed > MAX_OPERATOR_STARTUP_TIME) {
      Logger.warn("Completed operator startup in " + elapsedStr + " seconds.");
    } else {
      Logger.info("Completed operator startup in " + elapsedStr + " seconds.");
    }
      ...
      if (ybUniverse.getMetadata().getDeletionTimestamp() != null) {
        ...
        if (universeResp == null
          ...
          if (status.contains("DestroyKubernetesUniverse Success")) {
            ...
            Logger.info("Deleting provider now");
  ...
  private Result deleteUniverse(UUID customerUUID, UUID universeUUID) {
    ...
    if (!universe.getUniverseDetails().updateInProgress) {
      ...
      Logger.info("Delete in progress, not deleting universe");
      return null;
    }
  }

  private Result deleteProvider(UUID customerUUID, String universeName) {
    Logger.info("Deleting provider using operator");
    Customer customer = Customer.getOrBadRequest(customerUUID);
  ...
  private boolean canDeleteProvider(Customer customer, String universeName) {
    Logger.info("Checking if provider can be deleted");
    String providerName = getProviderName(universeName);
    Provider provider = Provider.get(customer.getUuid(), providerName, CloudType.kubernetes);
      ...
      if (currentUserIntent.kubernetesOperatorVersion != incomingIntent.kubernetesOperatorVersion) {
        ...
        if (!taskParams.updateInProgress) {
          ...
          if (deque != null) { // this should always be non-null
            ...
            Logger.info("Field being updated: " + field.getName());
            try {
              field.set(forTaskIntent, field.get(dequeUserIntent));
            } catch (Exception e) {
              Logger.error(
                  "Unable to set taskIntent for universe {}", forTaskIntent.universeName, e);
            }
          }
          ...
          if (!field.getName().equals("universeOverrides")) {
            ...
            Logger.info("removed overrides for upgrade/update");
          }
  ...
  private Result upgradeYBUniverse(
    ...
    try {
      ...
      Logger.error("Failed at creating upgrade software params", e);
    }
    ...
    Logger.info("Upgrading universe with new info now");
  ...
  private Result updateYBUniverse(
    ...
    try {
      ...
      Logger.error("Failed at creating configure task params for edit", e);
    }
  ...
  private UniverseConfigureTaskParams createTaskParams(YBUniverse ybUniverse, UUID customerUUID) {
    Logger.info("Creating task params");
    ...
    if (users.size() < 1) {
      Logger.error("Users list is of size 0!");
    } else {
      Logger.info("Taking first user for customer");
    }
  ...
  private UniverseConfigureTaskParams createTaskParams(UserIntent userIntent) {
    Logger.info("Creating task params from userIntent");
    UniverseConfigureTaskParams taskParams = new UniverseConfigureTaskParams();
  ...
  private UserIntent createUserIntent(YBUniverse ybUniverse, UUID customerUUID) {
    ...
    try {
      ...
      Logger.error("Unable to parse universe overrides", e);
    }
    Logger.info("YAML is:");
    Logger.info(userIntent.universeOverrides);
  ...
  private Provider getProvider(UUID customerUUID, YBUniverse ybUniverse) {
    Logger.info(ybUniverse.getSpec().toString());
    try {
      ...
      if (providers.isEmpty() && isRunningInKubernetes()) {
        ...
        Logger.info("region list is:");
        Logger.info(providerData.regionList.toString());
        ...
        Logger.info("Provider is already created, using that...");
        autoProvider = providers.get(0);
      }
      return Provider.getAll(customerUUID)
          .stream()
      ...
      if (isRunningInKubernetes()) {
        Logger.error("Running in k8s but no provider created", e);
      } else {
        Logger.info("Not running in k8s");
      }
      Logger.info("No automatic provider found, using first in provider list...");
      return Provider.getAll(customerUUID)
          .stream()
  ...
  private List<Pair<Field, UserIntent>> getDifferences(UserIntent oldIntent, UserIntent newIntent) {
    ...
    try {
      ...
      for (Field field : fields) {
        if (field.get(newIntent) != null && field.get(oldIntent) != null) {
          if (!field.get(oldIntent).equals(field.get(newIntent))) {
            ...
            Logger.info(field.get(oldIntent).toString());
            Logger.info(field.get(newIntent).toString());
            if (!field.getName().equals("kubernetesOperatorVersion")) {
      ...
      Logger.error("Failed to get differences for universe update", e);
    }
